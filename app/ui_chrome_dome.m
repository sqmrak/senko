...
/* tight specular blob on the upper left lobe. the heart path is inset by 6%
   and its left lobe crowns near 0.28 of the side, so a blob centred at 0.39
   sat in the valley between the lobes instead of on one of them */
    CGFloat sw = d * 0.30f, sh = d * 0.20f;
    CGFloat sx = d * 0.28f - sw * 0.5f, sy = d * 0.21f - sh * 0.5f;
...
/* frost is the UIVisualEffectView itself: it throws if a subview is added
   to it directly, so the tint has to live in its contentView instead. this
   is what crashed on every connect attempt with the glass theme active */
            UIView *frostHost = SenkoEffectContentView(frost);
            BOOL tinted = NO;
            for (UIView *sub in frostHost.subviews) {
                if (sub.tag == 9108) {
